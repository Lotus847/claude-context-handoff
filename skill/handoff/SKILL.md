...
```
By default (Windows) this adds a **new tab to the current Windows Terminal window** (`config.newSessionMode`; set `"window"` for a separate window), running `claude` named + seeded to read `HANDOFF.md` and carry out its NEXT ACTION — **in auto mode** for continuous workflow (`config.newSessionFlags`, default `--permission-mode auto`). On macOS/Linux (not yet auto-implemented) it prints the exact command to run manually. If `config.trustNewSessionFolder` is enabled it pre-accepts the folder's trust dialog (off by default — it edits a Claude Code security gate). Tip: add `--dry-run` to preview. Tell the user the new **named** session is open.

**8 — Offer to close THIS (old) session.** Report the context %, what memory you refreshed, and that the new session `<name>` is open, then **ask in chat**: *"Close this old session too?"*
- **If yes:** `node "{{RUNTIME}}/close-session.js"` — terminates only THIS session's host process (found via parent-walk; never sibling sessions). **Caveat:** it's a force-close, so Claude Code's own Stop/SessionEnd hooks may not run — if you rely on those, close the tab manually instead.
- **If no:** leave it open.
